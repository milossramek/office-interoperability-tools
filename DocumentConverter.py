...
# Configuration End #
#-------------------#
#class NoRegionsException(Exception):
...
if __name__ == "__main__":
    ...
    if len(argv) < 4:
        print "USAGE: python %s port <output-type> <input-file>" % argv[0]
        exit(255)
    if not isfile(argv[3]):
        print "no such input file: %s" % argv[3]
        exit(1)
    print argv
    fname, in_fmt = os.path.splitext(argv[3])
    print  "XX:", fname, in_fmt

    try:
        converter = DocumentConverter(argv[1])    
        converter.convert(argv[3], fname+'.'+argv[2])
